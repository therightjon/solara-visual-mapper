...
exports.selectorFromContext = selectorFromContext;
exports.findCodeLocation = findCodeLocation;
 ...
 */
function selectorFromContext(document, position) {
 ...
 */
function findNearestTagStreamingParser(document, position) {
    const stack = [];
    ...
    for (let lineNum = startLine; lineNum <= cursorLine; lineNum++) {
        ...
        if (lineNum === cursorLine) {
            ...
            if (lastIncompleteTag) {
                const completeTag = extractCompleteTag(document, lineNum, lastIncompleteTag.tagName, lastIncompleteTag.index);
                if (completeTag) {
 ...
 */
function extractCompleteTag(document, startLine, tagName, startColumn) {
    let tagContent = '';
 ...
 */
function extractSelectorFromText(text) {
    // Priority 1: data-code-id attribute
    const dataCodeIdMatch = text.match(/data-code-id\s*=\s*["']([^"']+)["']/) ||
    ...
    const classMatch = text.match(/\bclass(?:Name)?\s*=\s*["']([^"']+)["']/) ||
 ...
 */
function findCodeLocation(document, attributeType, value) {
...
//# sourceMappingURL=highlightMapping.js.map