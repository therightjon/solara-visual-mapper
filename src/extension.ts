...
import { selectorFromContext, findCodeLocation } from './highlightMapping';
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const setDevServerUrlCommand = vscode.commands.registerCommand(
    'visualPreview.setDevServerUrl',
    () => setDevServerUrl(context)
  );

  const selectStaticHtmlCommand = vscode.commands.registerCommand(
    'visualPreview.selectStaticHtml',
    () => selectStaticHtml(context)
  );
  ...
  context.subscriptions.push(
    openPreviewCommand,
    setDevServerUrlCommand,
    selectStaticHtmlCommand,
...
function openPreview(context: vscode.ExtensionContext) {
  ...
  const defaultMode = config.get<string>('defaultMode', 'currentFile');
...
async function updatePreviewContent(context: vscode.ExtensionContext, mode: string) {
  if (!previewPanel) {
    return;
  }

  const config = vscode.workspace.getConfiguration('visualPreview');
  const devServerUrl = config.get<string>('visualPreview.devServerUrl', '');
  ...
  try {
    if (mode === 'currentFile') {
      ...
      const actualDevServerUrl = config.get<string>('devServerUrl', '');
      ...
      const staticHtmlUriString = context.workspaceState.get<string>('visualPreview.staticHtmlUri');
...
async function setDevServerUrl(context: vscode.ExtensionContext) {
  ...
  if (url) {
    ...
    const switchMode = await vscode.window.showInformationMessage(
      'Dev server URL set. Switch to devServer mode?',
      'Yes',
      'No'
    );
...
async function selectStaticHtml(context: vscode.ExtensionContext) {
  ...
  if (uris && uris.length > 0) {
    ...
    const config = vscode.workspace.getConfiguration('visualPreview');
    const switchMode = await vscode.window.showInformationMessage(
      'Static HTML file selected. Switch to staticHtml mode?',
      'Yes',
      'No'
    );
...
function updateStatusBar(mode?: string) {
  ...
  const config = vscode.workspace.getConfiguration('visualPreview');
  const currentMode = mode || config.get<string>('defaultMode', 'currentFile');
